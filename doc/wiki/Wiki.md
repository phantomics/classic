...
- **Typed page subclasses** (e.g. `wiki-computer`, `wiki-cpu` with
  named MOP-annotated slots and lens-driven rendering). The demo uses
  an alist infobox; typed subclasses are a logical next step that
  would exercise the lens system in a model context.
...
mod/classic.models.common/
  wiki.lisp            -- wiki-page, wiki-revision classes;
                          link parser/resolver/healer; all wiki
                          operations; make-wiki preset
  workflows.lisp       -- editorial workflow (reused by wiki)
  accounts.lisp        -- publication-account (reused by wiki)
```

The wiki test suite lives in `test/test-wiki.lisp` (51 checks).
