...
Returns (values blog-a blog-b transport)."
  (let* ((blog-a (classic-blog:make-blog :name name-a
                                         :authority auth-a
                                         :authority-date "2026"))
         (blog-b (classic-blog:make-blog :name name-b
                                         :authority auth-b
                                         :authority-date "2026"))
         (transport (make-instance 'direct-transport)))
    ;; Configure federation on both blogs
    (setf (classic-blog::blog-transport blog-a) transport
          (classic-blog::blog-federation-roles blog-a) '(:publisher)
          (classic-blog::blog-transport blog-b) transport
          (classic-blog::blog-federation-roles blog-b) '(:aggregator))
    ;; Register with transport
    (register-with-transport transport (classic-blog:blog-publication blog-a))
    (register-with-transport transport (classic-blog:blog-publication blog-b))
    ;; Establish federation (bidirectional handshake)
    (establish-federation (classic-blog:blog-publication blog-a)
    ...
    (subscribe-to-feed (classic-blog:blog-publication blog-b)
...
(defun write-and-publish (blog title text)
  ...
  (let ((editor (classic-blog:create-account blog :name "Editor" :role :editor)))
    (classic-blog:write-post blog :account editor :title title :text text)
    (classic-blog:publish-post blog 1 :account editor)
    ;; Return the URI of the published post
    (uri-string (first (classic-blog:get-posts blog)))))
...
(test describe-instance-creates-descriptor
  ...
  (let* ((blog (classic-blog:make-blog :name "Test" :authority "test.dev"))
         (desc (describe-instance (classic-blog:blog-publication blog))))
...
(test register-peer-creates-peer-entity
  ...
  (let* ((blog (classic-blog:make-blog :name "Test" :authority "test.dev"))
         ...
         (peer (register-peer (classic-blog:blog-publication blog)
                              desc "remote.dev")))
...
(test establish-federation-registers-both-peers
  ...
  (let* ((blog-a (classic-blog:make-blog :name "A" :authority "a.dev"
                                         :authority-date "2026"))
         (blog-b (classic-blog:make-blog :name "B" :authority "b.dev"
                                         :authority-date "2026"))
         (transport (make-instance 'direct-transport)))
    (register-with-transport transport (classic-blog:blog-publication blog-a))
    (register-with-transport transport (classic-blog:blog-publication blog-b))
    ...
    (let ((a-has-peer nil) (b-has-peer nil))
      (maphash (lambda (uri entity)
                 ...
                 (when (typep entity 'classic-federation-peer)
                   (setf a-has-peer t)))
               (strategy-entities (classic-blog:blog-strategy blog-a)))
      (maphash (lambda (uri entity)
                 (declare (ignore uri))
                 (when (typep entity 'classic-federation-peer)
                   (setf b-has-peer t)))
               (strategy-entities (classic-blog:blog-strategy blog-b)))
      (is-true a-has-peer)
      (is-true b-has-peer))))
...
(test create-feed-stores-feed
  ...
  (let* ((blog (classic-blog:make-blog :name "Test" :authority "test.dev"))
         (feed (create-feed (classic-blog:blog-publication blog)
                            :type :all-published)))
...
(test federated-entity-has-provenance
  ...
  (multiple-value-bind (blog-a blog-b transport) (make-federated-pair)
    ...
    (let ((uri-a (write-and-publish blog-a "Provenance Test" "Content.")))
      (is-true (entity-federated-p (classic-blog:blog-publication blog-b) uri-a))
      (is (string= "alpha.dev"
...
(test non-subscribed-peer-does-not-receive
  ...
  (let* ((blog-a (classic-blog:make-blog :name "A" :authority "a.dev"
                                         :authority-date "2026"))
         (blog-c (classic-blog:make-blog :name "C" :authority "c.dev"
                                         :authority-date "2026"))
         (transport (make-instance 'direct-transport)))
    ;; Configure federation but DON'T subscribe C to A's feed
    (setf (classic-blog::blog-transport blog-a) transport
          (classic-blog::blog-federation-roles blog-a) '(:publisher)
          (classic-blog::blog-transport blog-c) transport
          (classic-blog::blog-federation-roles blog-c) '(:aggregator))
    (register-with-transport transport (classic-blog:blog-publication blog-a))
    (register-with-transport transport (classic-blog:blog-publication blog-c))
    (establish-federation (classic-blog:blog-publication blog-a)
...
(test resolve-entity-local-hit
  ...
  (let* ((blog (classic-blog:make-blog :name "Test" :authority "test.dev"))
         (editor (classic-blog:create-account blog :name "Ed" :role :editor)))
    (classic-blog:write-post blog :account editor :title "Local" :text "Here.")
    (let* ((posts (classic-blog:get-posts blog))
           (uri (uri-string (first posts)))
           (found (resolve-entity (classic-blog:blog-publication blog) uri)))
      (is-true found)
...
(test resolve-entity-peer-hit
  ...
  (multiple-value-bind (blog-a blog-b transport) (make-federated-pair)
    ...
    (let ((editor (classic-blog:create-account blog-a :name "Ed" :role :editor)))
      ...
      (let* ((posts (classic-blog:get-posts blog-a))
             (uri (uri-string (first posts))))
        ;; B doesn't have it locally
        (is-false (retrieve-entity (classic-blog:blog-strategy blog-b) uri nil))
        ;; But can resolve it via federation
        (let ((found (resolve-entity (classic-blog:blog-publication blog-b)
...
(test resolve-entity-returns-nil-on-miss
  ...
  (let* ((blog (classic-blog:make-blog :name "Test" :authority "test.dev"))
    ...
    (is-false (resolve-entity (classic-blog:blog-publication blog)
...
(test direct-transport-registers-instances
  ...
  (let* ((blog (classic-blog:make-blog :name "Test" :authority "test.dev"))
    ...
    (is (eq (classic-blog:blog-publication blog)
...
(test mixed-local-and-federated-content
  ...
  (multiple-value-bind (blog-a blog-b transport) (make-federated-pair)
    ...
    (let ((editor-b (classic-blog:create-account blog-b :name "Bob" :role :editor)))
