...
(test make-blog-has-publication
  ...
  (let ((blog (make-test-blog :name "My Blog")))
    (is (typep (classic.models.common:blog-publication blog) 'classic-publication))
    (is (string= "My Blog"
                  (classic.schema.alpha:label (classic.models.common:blog-publication blog))))))

(test make-blog-has-empty-container
  "Blog has a container with empty contains list."
  (let ((blog (make-test-blog)))
    (is (typep (classic.models.common:blog-container blog) 'classic-container))
    (is (null (classic.schema.alpha:contains (classic.models.common:blog-container blog))))))

(test make-blog-has-workflow
  "Blog has a workflow with draft initial state."
  (let ((blog (make-test-blog)))
    (is (typep (classic.models.common:blog-workflow blog) 'classic-workflow))
    (is (string= "draft" (initial-state (classic.models.common:blog-workflow blog))))))

(test make-blog-has-roles
  "Blog has writer and editor roles."
  (let* ((blog (make-test-blog))
         (roles (classic.models.common:blog-roles blog)))
...
(test create-account-writer
  ...
  (let ((blog (make-test-blog)))
    (multiple-value-bind (writer) (make-test-accounts blog)
      (is (typep writer 'classic.models.common:blog-account))
...
(test create-account-editor
  ...
  (let ((blog (make-test-blog)))
    (multiple-value-bind (writer editor) (make-test-accounts blog)
      ...
      (is (typep editor 'classic.models.common:blog-account))
...
(test write-post-returns-uri
  ...
  (let ((blog (make-test-blog)))
    (multiple-value-bind (writer) (make-test-accounts blog)
      (let ((uri (classic.models.common:write-post blog
                                          :account writer
                                          :title "Test Post"
                                          :text "Content")))
        (is (stringp uri))
  ...
  (let ((blog (make-test-blog)))
    (multiple-value-bind (writer) (make-test-accounts blog)
      ...
      (let* ((post (first (classic.models.common:get-posts blog)))
        ...
        (let ((person (retrieve-entity (classic.models.common:blog-strategy blog)
                                       author-uri nil)))
...
(test writer-cannot-publish
  ...
  (let ((blog (make-test-blog)))
    (multiple-value-bind (writer) (make-test-accounts blog)
      ...
      (classic.models.common:publish-post blog 1 :account writer)
      (let ((post (first (classic.models.common:get-posts blog))))
...
(test publish-already-published-fails
  ...
  (let ((blog (make-test-blog)))
    (multiple-value-bind (writer editor) (make-test-accounts blog)
      ...
      (finishes (classic.models.common:publish-post blog 1 :account editor))
      ;; Post should still be published
      (is (string= "published"
                    (current-state (first (classic.models.common:get-posts blog))))))))
...
(test get-posts-returns-all
  ...
  (let ((blog (make-test-blog)))
    (multiple-value-bind (writer editor) (make-test-accounts blog)
      (classic.models.common:write-post blog :account writer :title "P1" :text "A")
      (classic.models.common:write-post blog :account writer :title "P2" :text "B")
      (classic.models.common:publish-post blog 1 :account editor)
      (is (= 2 (length (classic.models.common:get-posts blog)))))))
...
(test get-posts-filters-published
  ...
  (let ((blog (make-test-blog)))
    (multiple-value-bind (writer editor) (make-test-accounts blog)
      (classic.models.common:write-post blog :account writer :title "P1" :text "A")
      (classic.models.common:write-post blog :account writer :title "P2" :text "B")
      (classic.models.common:publish-post blog 1 :account editor) ; publishes P2 (newest)
      (let ((published (classic.models.common:get-posts blog :status "published")))
...
(test get-posts-filters-drafts
  ...
  (let ((blog (make-test-blog)))
    (multiple-value-bind (writer editor) (make-test-accounts blog)
      ...
      (classic.models.common:write-post blog :account writer :title "P2" :text "B")
      (classic.models.common:publish-post blog 1 :account editor) ; publishes P2 (newest)
      (let ((drafts (classic.models.common:get-posts blog :status "draft")))
        (is (= 1 (length drafts)))
...
(test posts-ordered-newest-first
  ...
  (let ((blog (make-test-blog)))
    (multiple-value-bind (writer) (make-test-accounts blog)
      (classic.models.common:write-post blog :account writer :title "First" :text "A")
      (classic.models.common:write-post blog :account writer :title "Second" :text "B")
      (let ((posts (classic.models.common:get-posts blog)))
...
(test show-post-valid-index
  ...
  (let ((blog (make-test-blog)))
    (multiple-value-bind (writer) (make-test-accounts blog)
      (classic.models.common:write-post blog :account writer :title "Show Me" :text "Content")
      (let ((post (classic.models.common:show-post blog 1)))
        (is-true post)
        (is (typep post 'classic.models.common:blog-article))))))

(test show-post-invalid-index
  "show-post returns nil for invalid index."
  (let ((blog (make-test-blog)))
    (multiple-value-bind (writer) (make-test-accounts blog)
      (classic.models.common:write-post blog :account writer :title "Solo" :text "Content")
      (is-false (classic.models.common:show-post blog 99)))))
